...
function createBubbleChart1(error, dataset) {
  ...
  var width = 1000,
    ...
    console.log(grouping_option);
    
    var clusterType = getClusterType1();
    ...
    if(clusterType == "Geo_Cluster") {
      domain_type = d3.set(dataset.map(function(country) { return country.Geo_Cluster; }));
    } else if(clusterType == "direction") {
      domain_type = d3.set(dataset.map(function(country) { return country.Direction_Cluster; }));
    } else if(clusterType == "time") {
      domain_type = d3.set(dataset.map(function(country) { return country.Time_Cluster; }));
    }
    ...
    if(clusterType == "Geo_Cluster") {
      ...
      cluster_domain = d3.nest()
        .key(function(d) { return d.Direction_Cluster; })
        .rollup(function(v) { return {
          count: v.length,
        }; })
        .entries(dataset);
      ...
      cluster_domain = d3.nest()
        .key(function(d) { return d.Time_Cluster; })
        .rollup(function(v) { return {
          count: v.length,
        }; })
        .entries(dataset);
    ...
    if(grouping_option == "combine") {
    ...
    } else if(grouping_option == "cluster") {
      ...
      forceX = d3.forceX(function(d) {
        ...
        if(clusterType == "Geo_Cluster") {
          data_cluster_key = d.Geo_Cluster;
        } else if(clusterType == "Geo_Cluster") {
          data_cluster_key = d.Direction_Cluster;
        } else if(clusterType == "Geo_Cluster") {
          data_cluster_key = d.Time_Cluster;
        }
      ...
      forceY = d3.forceY(function(d) {
        ...
        for(var i = 0; i < cluster_keys.length; i++) {
          ...
          if(data_cluster_key == cluster_keys[i]) {
            force_val = ((height/num_rows) * (count)) - 200;
            break;
          }
    ...
    } else if(grouping_option == "country-centers") {
    ...
    if(clusterType == "Geo_Cluster") {
      circles = svg.selectAll(".countries")
        .data(dataset)
        .enter().append("circle")
        .attr("class","country")
        .attr("r", function(d) {
        ...
        .attr("fill",function(d) {
          return clusterColorScale(d.Geo_Cluster);
        });
    } else if(clusterType == "direction") {
      circles = svg.selectAll(".countries")
        .data(dataset)
        ...
        .attr("fill",function(d) {
          return clusterColorScale(d.Direction_Cluster);
        });
    } else if(clusterType == "time") {
      circles = svg.selectAll(".countries")
        .data(dataset)
        ...
        .attr("fill",function(d) {
          return clusterColorScale(d.Time_Cluster);
        });
    }
